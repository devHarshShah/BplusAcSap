...
export async function POST(req: NextRequest, res: NextResponse) {
  try {
    const { timeSheetId, weekEntries } = await req.json(); // Extract employeeCode and mondayDate from request

    await connectMongo(); // Connect to MongoDB

    // Assuming weekEntries is an object like { hoursWorked: 40, project: "New Project" }
    const timeSheetUpdated = await TimesheetWeek.findByIdAndUpdate(timeSheetId, { weekEntries: weekEntries }, { new: true });
